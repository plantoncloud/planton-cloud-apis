package buf.gen.cloud.planton.apis.code2cloud.v1.deploy.kubernetes.locustkubernetes;

import build.buf.gen.cloud.planton.apis.code2cloud.v1.deploy.kubernetes.locustkubernetes.model.LocustKubernetes;
...
public final class LocustKubernetesTest {
    ...
    public void testLocustKubernetes_ShouldReturnValidationErrorIfMetadataIsNotPassed() throws ValidationException {
        var locustKubernetes  = LocustKubernetes.newBuilder().build();
        Validator validator = new Validator();
        var result = validator.validate(locustKubernetes );
    ...
    public void testLocustKubernetes_ShouldReturnValidationErrorIfVersionMessageIsNotPassed() throws ValidationException {
        var locustKubernetes  = LocustKubernetes.newBuilder().setMetadata(ApiResourceMetadata.newBuilder().build()).build();
        Validator validator = new Validator();
        var result = validator.validate(locustKubernetes );
        var versionMessageViolation = result.getViolations().stream()
    ...
    public void testLocustKubernetes_ShouldNotReturnValidationErrorIfVersionMessageIsPassed() throws ValidationException {
        var locustKubernetes  = LocustKubernetes.newBuilder()
                .setMetadata(ApiResourceMetadata.newBuilder()
                        .setVersion(ApiResourceMetadataVersion.newBuilder().setMessage(" test locust-kubernetes ").build())
                        .build())
                .build();
        Validator validator = new Validator();
        var result = validator.validate(locustKubernetes );
