package buf.gen.cloud.planton.apis.code2cloud.v1.rediskubernetes.rpc;

import build.buf.gen.cloud.planton.apis.code2cloud.v1.kubecluster.model.IngressSpec;
import build.buf.gen.cloud.planton.apis.code2cloud.v1.rediskubernetes.model.RedisKubernetes;
...
public final class RedisKubernetesTest {
    ...
    public void testRedisKubernetes_ShouldReturnValidationErrorIfMetadataIsNotPassed() throws ValidationException {
        var redisCluster = RedisKubernetes.newBuilder().build();
        Validator validator = new Validator();
    ...
    public void testRedisKubernetes_ShouldReturnValidationErrorIfVersionMessageIsNotPassed() throws ValidationException {
        var redisCluster = RedisKubernetes.newBuilder().setMetadata(ApiResourceMetadata.newBuilder().build()).build();
        Validator validator = new Validator();
        var result = validator.validate(redisCluster);
        var versionMessageViolation = result.getViolations().stream()
    ...
    public void testRedisKubernetes_ShouldNotReturnValidationErrorIfVersionMessageIsPassed() throws ValidationException {
        var redisCluster = RedisKubernetes.newBuilder()
